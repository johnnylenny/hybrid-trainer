...
All fields are strings, all are optional. Units are whatever the user typed.
...
- **v4** (current) — Added `schemaVersion`, `exportedAt`, optional `rir` field on working sets, and `settings` to exports.
