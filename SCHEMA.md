...
- **v17** (current) — Garmin watch strength import (app v0.58.0, `sync/garmin_sync.py`). Completed `strength_training` activities (previously skipped entirely) now import as `lifts` sessions, review-first: a new recognized key, `condData.reviewPending` (boolean, present only until the owner opens the session once), is the whole shape change. No DB migration (`cond_data` is jsonb) — same reasoning as every prior `cond_data`/`run_data` addition below. Old sessions and hand-logged lifts sessions never carry this key.
- **v16** — The optional per-set `done` boolean (app v0.31.0), documented in full under "Set object" above. Missed in this changelog when it shipped; added here in passing while touching this section for v17 (`hybrid-trainer-change-control`'s Gate 4: fix doc drift when you find it). No DB migration (sets live in the `exercises` jsonb).
- **v15** — External data import. Adds a session-level `importSource` field (and an `import_source` text column on the `sessions` table — one-line migration, see the `sessions` table section) so the same imported workout isn't added twice on re-import. Format `<source>:<id>`. First consumer is the **Hevy CSV import** (app v0.25.0), which sets `hevy:<start_time>|<title>` and maps each Hevy workout to a `lifts` session (one exercise per `exercise_title`, so supersets split; each drop-set row becomes its own working set). Additive and backwards compatible — old sessions have no `importSource` (treated as empty); the DB column is nullable. The field is generic and reusable by future GPX/TCX/FIT/Strava imports. (The originally-planned first source was GPX/TCX; Hevy CSV shipped first because it maps cleanly to the existing lifts model with no new dependency.)
